...
public final class ArrayTrieHashMap<K, V> implements Map<K, V> {
  ...
  private static final class Tree<K, V>
    ...
    @Override
    public List<Entry<K, V>> list() {
      return new ListParent<K, V>(null, this, 0).findLeaf();
    }
  ...
  private static final class ListParent<K, V> {
    ...
    ListParent(ListParent<K, V> parent, Tree<K, V> tree, int index) {
      this.parent = parent;
      this.tree = tree;
      this.index = index;
    }
    ...
    List<Entry<K, V>> findLeaf() {
      ...
      if (node instanceof Tree) {
        return new ListParent<K, V>(this, (Tree<K, V>) node, 0).findLeaf();
      }
      else {
    ...
    List<Entry<K, V>> tail() {
      if (index + 1 < tree.nodes.length) {
        return new ListParent<K, V>(parent, tree, index + 1).findLeaf();
      }
      if (parent != null) {
        return parent.tail();
      }
